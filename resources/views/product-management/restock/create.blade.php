...
@section('title')
    @lang('product-management.page_title_product_mgmt_restock_create')
@stop
            ...
            <div class="col-xs-12">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title">@lang('product-management.box_header_restock_create')</h3>
                    </div><!-- /.box-header -->
                    <div class="box-body create-restock create-restock-create-page">
                        {!! Form::open(array('id' => 'restock_product_form', 'url' => route('products.restock.store'), 'method' => 'POST')) !!}
                            <div class="col-xs-12">
                                <div class="col-xs-6">
                                    <div class="form-group has-feedback">
                                        <label class="col-xs-3 control-label required" for="merchandiser">@lang('product-management.restock_form_label_merchandiser')</label>
                                        <div class="col-xs-9">
                                            {!! Form::select('merchandiser', $merchandiser, null, array('class' => 'form-control select2', 'placeholder' => trans('product-management.restock_form_placeholder_merchandiser'))) !!}
                                            <div class="error">{{ $errors->first('merchandiser') }}</div>
                                        </div>
                                    </div>

                                    <div class="form-group has-feedback">
                                        <label class="col-xs-3 control-label required" for="batch_date">@lang('product-management.restock_form_label_batch_date')</label>
                                        <div class="col-xs-9">
                                            {!! Form::text( 'batch_date', null, ['id' => 'batch_date_input', 'class' => 'form-control', 'placeholder' => trans('product-management.restock_form_placeholder_batch_date')] ) !!}
                                            <div class="error">{{ $errors->first('batch_date') }}</div>
                                        </div>
                                    </div>

                                    <div class="form-group has-feedback">
                                        <label class="col-xs-3 control-label required" for="merchant">@lang('product-management.restock_form_label_merchant')</label>
                                        <div class="col-xs-9">
                                            {!! Form::select('merchant', $merchant, null, array('class' => 'form-control select2', 'id' => 'merchant_dropdown', 'placeholder' => trans('product-management.restock_form_placeholder_merchant'))) !!}
                                            <div class="error">{{ $errors->first('merchant') }}</div>
                                        </div>
                                    </div>

                                    <div class="form-group has-feedback">
                                        <label class="col-xs-3 control-label required" for="channel">@lang('product-management.restock_form_label_supplier')</label>
                                        <div class="col-xs-9">
                                            {!! Form::select('supplier', array(), null, array('class' => 'form-control select2', 'id' => 'supplier_dropdown', 'placeholder' => trans('product-management.restock_form_placeholder_supplier'))) !!}
                                            <div class="error">{{ $errors->first('supplier') }}</div>
                                        </div>
                                    </div>

                                    <div class="form-group has-feedback">
                                        <label class="col-xs-3 control-label required" for="channel">@lang('product-management.restock_form_label_channel')</label>
                                        <div class="col-xs-9">
                                            {!! Form::select('channel', array(), null, array('class' => 'form-control select2', 'id' => 'channel_dropdown', 'placeholder' => trans('product-management.restock_form_placeholder_channel'))) !!}
                                            <div class="error">{{ $errors->first('channel') }}</div>
                                        </div>
                                    </div>

                                    <div class="form-group has-feedback">
                                        <label class="col-xs-3" for="remarks">@lang('product-management.restock_form_label_remarks')</label>
                                        <div class="col-xs-9">
                                            {!! Form::textarea( 'remarks', null, ['class' => 'form-control', 'placeholder' => trans('product-management.restock_form_placeholder_remarks')] ) !!}
                                ...
                                <div class="col-xs-6">
                                    <div class="col-xs-12">
                                        <div class="form-group has-feedback pull-right">
                                            <button type="button" id="btn_download_restock" class="btn btn-default">@lang('product-management.button_download_restock_template')</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-xs-12">
                                <div class="form-group pull-right">
                                    <div class="col-xs-6">
                                        <button data-toggle="modal" data-target="#uploadModal" type="button" id="btn_upload_restock" class="btn btn-default">@lang('product-management.button_upload_restock_sheet')</button>
                                    </div>
                                </div>
                            </div>
                        {!! Form::close() !!}
                        ...
                        {!! Form::open(array('id'=>'download-csv', 'url' => route('products.download.csv'), 'method' => 'POST')) !!}
                            ...
                            <input type="hidden" name="filename" value="replenishment_form">
                        {!! Form::close() !!}
                    </div>
                </div>
            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Upload Restock Product Sheet</h4>
                </div>
                <div class="modal-body">
                    ...
                    <span class="upload-form">
                        ...
                        <p class="help-block">Upload your restock product sheet here.</p>
                    </span>
    ...
    $(document).ready(function(){
            ...
            if($(this).val() > 0){
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if(data.channels==null || data == null || data == ''){
                            // show placeholder
        ...
        $('#file_upload').fileupload({
            ...
            add: function (e, data) {
                ...
                data.formData = $("#restock_product_form").serializeArray();
                data.submit();
            },
                ...
                if(result.success){
                    if(result.existed){
                        ...
                        $('<p/>').text('Restock product batch created successfully! Now redirecting to the edit page...').appendTo('#log');
        ...
        $("#btn_download_restock").click(function() {
            $("#download-csv").submit();
        });
    });
</script>
