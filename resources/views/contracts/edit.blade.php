...
@section('title')
	@lang('contracts.page_title_contracts_edit')
@stop
	        ...
	        <div class="col-xs-12">
	          	<div class="box">
	            	<div class="box-header">
	              		<h3 class="box-title">@lang('contracts.box_header_contracts_edit')</h3>
	            	</div><!-- /.box-header -->
	            	<div class="box-body contracts-module contract-edit-page">
	            		{!! Form::open(array('url' => route('contracts.update', $contract->id), 'method' => 'PUT', 'id' => 'edit-contract-form')) !!}
		            		...
		            		<div class="row">
		            			<div class="form-group">
			            			...
			            			<div class="col-xs-5">
			            				{!! Form::select('merchant', $merchants, (!empty(Input::old('merchant')))?Input::old('merchant'):$contract->merchant_id, array('class' => 'form-control select2', 'id' => 'merchant_dropdown', 'placeholder' => trans('contracts.create_placeholder_merchant'))) !!}
		            			...
		            			<div class="form-group">
			            			...
			            			<div class="col-xs-5">
			            				<div class="row radio">
			            					<label>
					            				<div class="col-xs-1">
					            					<input style="margin-left: -8px;" type="radio" name="minimum_guarantee" value="not_applicable" {{ ((!isset($old_inputs['minimum_guarantee']) || $old_inputs['minimum_guarantee'] == 'not_applicable')) ? 'checked' : (is_null($contract->guarantee))?'checked':'' }}>
					            				</div>
			            			...
			            			<div class="col-xs-5">
			            				<div class="row">
			            					...
			            					<div class="col-xs-12 error">
			            						@lang('contracts.date_note_1') <a href="{{ route('contracts.duplicate', [$contract->id]) }}">@lang('contracts.date_note_2')</a>.
			            					</div>
			            					@endif
			            				</div>
			            			</div>
		            			...
		            			<div class="form-group">
			            			...
			            			<div class="col-xs-5">
			            				<div class="row">
			            					<div class="col-xs-6">
			            						<div class="input-group">
				            						{!! Form::text( 'return_fee', $contract->return_fee, ['class' => 'form-control', 'placeholder' => trans('contracts.create_placeholder_return_fee')] ) !!}
			            						</div>
		            			...
		            			<div class="form-group">
			            			...
			            			<div class="col-xs-5">
			            				<div class="row">
			            					<div class="col-xs-6">
			            						<div class="input-group">
				            						{!! Form::text( 'shipped_fee', $contract->shipped_fee, ['class' => 'form-control', 'placeholder' => trans('contracts.create_placeholder_shipped_fee')] ) !!}
			            						</div>
		            			...
		            			<div class="form-group">
			            			...
			            			<div class="col-xs-5">
			            				<div class="row">
			            					<div class="col-xs-6">
			            						<div class="input-group">
				            						{!! Form::text( 'storage_fee', $contract->storage_fee, ['class' => 'form-control', 'placeholder' => trans('contracts.create_placeholder_storage_fee')] ) !!}
			            						</div>
		            			...
		            			<div class="form-group">
			            			...
			            			<div class="col-xs-5">
			            				<div class="row">
			            					<div class="col-xs-6">
			            						<div class="input-group">
				            						{!! Form::text( 'inbound_fee', $contract->inbound_fee, ['class' => 'form-control', 'placeholder' => trans('contracts.create_placeholder_inbound_fee')] ) !!}
			            						</div>
		            			...
		            			<div class="form-group">
			            			...
			            			<div class="col-xs-5">
			            				<div class="row">
			            					<div class="col-xs-6">
			            						<div class="input-group">
				            						{!! Form::text( 'outbound_fee', $contract->outbound_fee, ['class' => 'form-control', 'placeholder' => trans('contracts.create_placeholder_outbound_fee')] ) !!}
			            						</div>
			            			...
			            			<div class="form-group">
				            			...
				            			<div class="col-xs-10 contract-fee-container">
				            				...
				            				@if(empty($old_inputs['fee-info']))
				            					@foreach($contract->contract_rules as $index => $rule)
						            				...
						            				<div class="row">
					            						...
					            						<div class="row filters-row">
					            							<div class="col-xs-12">
					            								{!! Form::select('fee-info['.$index.'][channel][]', $channels, json_decode($rule->channels), array('class' => 'form-control select2-tags-channel', 'multiple' => true)) !!}
					            							</div>
					            						</div>
					            						<div class="row filters-row">
					            							<div class="col-xs-12">
					            				...
					            				@foreach($old_inputs['fee-info'] as $index => $oldInput)
						            				<div class="contract-fee" data-index="{{$index}}" id="contract-fee-{{ $index }}">
							            				...
							            				<div class="row fee-options">
							            					...
							            					<div class="custom-textbox">
								            					<div class="input-group">
								            						<span class="input-group-addon type-symbol">@if($oldInput['type']=='Percentage')%@elseif($oldInput['type']=='Fixed Rate')$@endif</span>
	            		...
	            		{!! Form::close() !!}
	            		<form action="{{route('contracts.destroy', [$contract->id])}}" id="delete-form" method="POST">
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		var rowIndex = {{ (isset($old_inputs['fee-info'])) ? (int)(key( array_slice( $old_inputs['fee-info'], -1, 1, TRUE )) + 1 ) : count($contract->contract_rules) }};
		var currentChannels = '';
		var currentProducts = '';
		...
		$.each({!! json_encode($channels) !!}, function( index, channel ) {
			currentChannels += '<option value="'+index+'">'+channel+'</option>';
		});
		@endif
		...
		$.each({!! json_encode($products) !!}, function( index, product ) {
			currentProducts += '<option value="'+index+'">'+product+'</option>';
		});
		@endif
		...
		$('#btn-add-new-rule').click(function(){
            var row = '\
            			<div class="contract-fee" data-index="'+rowIndex+'" id="contract-fee-'+rowIndex+'">\
    						...
    						<div class="row filters-row">\
    							<div class="col-xs-12">\
    								{!! Form::select('fee-info[rowIndex][product][]', array(), null, array('class' => 'form-control select2-tags-product', 'multiple' => true)) !!}\
    							</div>\
    						</div>\
            				<div class="row">\
            					<div class="col-xs-12">\
