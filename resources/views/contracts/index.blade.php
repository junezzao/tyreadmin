...
@section('title')
	@lang('contracts.page_title_contracts_index')
@stop
	        ...
	        <div class="col-xs-12">
	          	<div class="box">
	            	<div class="box-header">
	              		<h3 class="box-title">@lang('contracts.box_header_contracts_index')</h3>
	              		@if($user->can('create.contract'))
                        <a href="{!! route('contracts.create', []) !!}" id="new_contract" class="btn btn-default pull-right">
                            @lang('contracts.btn_create_new')
                        </a>
                        @endif
	            	</div><!-- /.box-header -->
	            		...
	            		<table id="contracts-table" class="table table-bordered table-striped">
		                    <thead>
		                    	...
		                    	<tr class="search-row">
		                            ...
		                            <td style="width: 11%" class="search text 2"></td>
		                            <td style="width: 10%" class="search text 3"></td>
...
jQuery(document).ready(function(){
	...
	var table = jQuery('#contracts-table').DataTable({
		...
		"columnDefs": [
            ...
            { "data": "merchant", "name": "merchant", "targets": 1 },
            { "data": "brand", "name": "brand", "targets": 2},
            { "data": "name", "name": "name", "targets": 3},
            { "data": "created_at", "name": "created_at", "targets": 4 }, 
            { "data": "updated_at", "name": "updated_at", "targets": 5 }, 
            { "data": "start_date", "name": "start_date", "targets": 6 },
            { "data": "end_date", "name": "end_date", "targets": 7 },
