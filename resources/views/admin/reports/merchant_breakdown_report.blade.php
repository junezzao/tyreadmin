                ...
                <div class="box">
                    ...
                    <div class="box-body generate-report-container">
                        ...
                        <div class="merchant-breakdown-report-data">
                            <div class="row">
                                <h4 class="pull-left">
                                    <span class="merchant-name"></span> Performance Report (<span class="selected-report-date-range"></span>) 
                                </h4>
                                <div class="pull-right">
                                    {!! Form::open(['url' => '/admin/generate-report/export', 'method' => 'POST', 'id' => 'export']) !!}
                                        {!! Form::hidden('data', json_encode($hwSkus)) !!}
                                        {!! Form::hidden('type', '') !!}
                            ...
                            <div class="row chart-div">
                                <div class="col-md-6 col-xs-12">
                                    <div class="breakdown-data col-md-10">
                                        <div class="row">
                                            <div class="col-xs-7">
                                                No. of Products Out of Stock:
                                            </div>
                                            <div class="col-xs-5">
                                                {{ $summaryData->out_of_stock_products }}
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-xs-7">
                                            ...
                                            <div class="col-xs-5">
                                                {{ isset($summaryData->total_num_and_value_items_sold[0]->count) ? $summaryData->total_num_and_value_items_sold[0]->count : 0 }}
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-xs-7">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-xs-5">
                                                {{ isset($summaryData->total_num_and_value_items_sold[0]->sum) ? 'MYR ' . number_format((float)$summaryData->total_num_and_value_items_sold[0]->sum, 2, '.', ',') : 'MYR 0.00' }}
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-xs-7">
                                            ...
                                            <div class="col-xs-5">
                                                {{ isset($summaryData->channel_performance->most_value_sold[0]->name) ? $summaryData->channel_performance->most_value_sold[0]->name : 'N/A' }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6 col-xs-12">
                                    <div class="chart-responsive">
                                        <div class="chart donut-chart" id="performance-in-channel-chart"></div>
                                    </div>
                                    <div class="donut-chart-label">
                                        <h3>Performance in Channels</h3>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                    ...
                                    <table id="reports-table" class="table table-bordered table-striped">
                                        <thead>
                                            ...
                                            <tr>
                                                ...
                                                <td data-index="1" class="search-col-dd"></td>
                                                <td data-index="2" class="search-col-dd"></td>
                                        ...
                                        <tbody>
                                            @foreach($hwSkus as $data)
                                                <tr>
                                                    <td>{{ $data->hubwire_sku }}</td>
...
$(document).ready(function() {
    ...
    // Set report title
    $('.merchant-name').html($('select[name="merchant[]"]').find('option:selected').text()); 
    $('input[name="type"]').val($('select[name="merchant[]"]').find('option:selected').text()+' Performance');
    @if(isset($selectedDate))
        $('.selected-report-date-range').html('{{ $selectedDate }}');
    @endif
    @if($showCharts)
    //Donut Chart
    //number_format((float)$item->item_cost, 2, '.', '')
    var top10ProductsDonut = new Morris.Donut({
        element: 'performance-in-channel-chart',
        resize: true,
        colors: [
            @if(!empty($summaryData->num_and_value_items_sold_per_channel))
                @foreach($summaryData->num_and_value_items_sold_per_channel as $data)
                    "{{ $data->colourCode }}",
                @endforeach
            @else
                "#d2d6de",
        ...
        data: [
            @if(!empty($summaryData->num_and_value_items_sold_per_channel))
                @foreach($summaryData->num_and_value_items_sold_per_channel as $data)
                    {label: "{{ $data->name . ' ['.$data->count.']' }}", value: {{ $data->sum }}},
                @endforeach
            @else
    ...
    var reportsTable = jQuery('#reports-table').DataTable({
        ...
        "orderCellsTop": true,
        "columns": [
            { "data": "hwSku", "name": "hwSku", "targets": 0 },
            { "data": "brand", "name": "brand", "targets": 1 },
            { "data": "category", "name": "category", "targets": 2 },
            { "data": "qty", "name": "qty", "targets": 3 },
            { "data": "availableQty", "name": "availableQty", "targets": 4 }, 
            { "data": "sold", "name": "sold", "targets": 5 },
            { "data": "gmv", "name": "gmv", "targets": 6 },
            { "data": "returnsInTransit", "name": "returnsInTransit", "targets": 7 },
            { "data": "returnsCompleted", "name": "returnsCompleted", "targets": 8 },
            { "data": "cancelled", "name": "cancelled", "targets": 9 }
        ],
