...
trait HasRoleAndPermission
{
    ...
    /**
     * Check if the user has role(s) other than a specific role.
     *
     * @param int|string $role
     * @return bool
     */
    public function hasRoleOtherThan($role)
    {
        return $this->getRoles()->contains(function ($key, $value) use ($role) {
            if($role != $value->id && !Str::is($role, $value->slug))
                return true;
        });
        return false;
    }
    ...
    {
        $userModel = app(config('roles.models.user'));
        return $userModel::with('channel')->find($this->id)->channel;
    }
}
